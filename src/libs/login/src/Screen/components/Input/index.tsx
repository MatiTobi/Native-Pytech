...
export default memo(({
    ...
    const [errorMessage, setErrorMessage] = useState<string | null>(messageError || null)
	...
	const _onBlur = () => {
        setIsFocused(false)
        if (!valueRef.current) animShared.value = withTiming(0, {...anim, duration: 75})
        onBlur?.()
	}
    ...
    const _handleSubmit = async () => {
        ...
        if (!succeded) setErrorMessage(message)
    ...
    return (
        <View style={{gap: 10}}>
            <View 
                ...
                style={[styles.container, {borderColor: borderColor, backgroundColor: backgroundColor}]}
            >
                <Pressable style={styles.pressable} onPress={() => {inputRef?.current?.focus?.()}}>
                    ...
                    <TextInput style={[styles.textInput, {color: Theme.text}]}
                        autoCapitalize='none'
                        autoCorrect={false}
                        ...
                        onFocus={_onFocus}
                        onBlur={_onBlur}
                        onSubmitEditing={onClickSubmit}
                    />
                </Pressable>

                {buttonSend && (
                    !isLoading ? (
                        <Pressable style={{paddingHorizontal: 10}} onPress={onClickSubmit} onTouchStart={onClickSubmit}>
