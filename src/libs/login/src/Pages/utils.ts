...
export const getAbbreviatedName = ({firstname, lastname, mail}: {firstname?: string, lastname?: string, mail: string}) => {
    ...
    let name = ''
    if (f && l) name = f + l
    else if (f) name = f + (firstname?.[1] ?? f)
    else if (l) name = l + (lastname?.[1] ?? l)
