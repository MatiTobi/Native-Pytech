...
export const scrollToIndex = ({ widths, scrollX, containerWidth, scrollRef, prevIndex, index }) => {
    ...
    if (index > prevIndex) {
        if (nextWidth > 0) {
            ...
            const isNextFullyVisible = nextStart >= visibleStart &&
                nextEnd <= visibleEnd;
    ...
    // 👉 dirección izquierda
        ...
        if (prevWidth > 0) {
            ...
            const prevEnd = left;
            const isPrevFullyVisible = prevStart >= visibleStart &&
                prevEnd <= visibleEnd;
