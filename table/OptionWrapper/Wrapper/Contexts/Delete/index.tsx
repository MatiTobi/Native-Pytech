...
import { useLayoutEffectWithoutFirstRender, Colors } from '@react-native-pytech/constants'
...
export default memo(({ children, ...props }: Omit<DeleteProps, 'setDeleted'>) => {
...
const Component = memo(({
    ...
    const onLayputProps = removeWidth ? {} :{ onLayout: (e: LayoutChangeEvent) => setWidthRemove(e.nativeEvent.layout.width) }
    const styleWidthRemove = removeWidth ? {width: removeWidth} : {}


    // Función que los children pueden llamar para iniciar el delete
    const startDelete = useCallback(() => {
        if (store.deleted.setted.peek()) return
    ...
    const start = useCallback(() => {
        if (store.deleted.id.peek() === null) return
